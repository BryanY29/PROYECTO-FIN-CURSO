...
conn = sqlite3.connect('BD_ALARMA_POMODORO2.db')
cursor = conn.cursor()
...
class Alarm(QWidget):
    ...
    def record_alarm_set(self):
        cursor.execute('''INSERT INTO "tbl.Alarma" (Hora_de_alarma, tiempo_Transcurrido) VALUES (?, ?)''', (self.alarm_time.toString(), 0))
        conn.commit()

    def record_alarm_triggered(self):
        cursor.execute('''UPDATE "tbl.Alarma" SET tiempo_Transcurrido = ? WHERE ID = (SELECT MAX(ID) FROM "tbl.Alarma")''', (self.alarm_time.toString(),))
        conn.commit()
...
class Pomodoro(QWidget):
    ...
    def record_pomodoro_start(self):
        cursor.execute('''INSERT INTO "tbl.Pomodoro" (Hora_Inicio, Hora_Descanso) VALUES (?, ?)''', (QTime.currentTime().toString(), ''))
        conn.commit()

    def record_pomodoro_stop(self):
        cursor.execute('''UPDATE "tbl.Pomodoro" SET Hora_Descanso = ? WHERE ID = (SELECT MAX(ID) FROM "tbl.Pomodoro")''', (QTime.currentTime().toString(),))
        conn.commit()
