...
cursor.execute('''CREATE TABLE IF NOT EXISTS pomodoros (
                   id INTEGER PRIMARY KEY AUTOINCREMENT,
                   type TEXT,
                   time_elapsed INTEGER,
                   is_working INTEGER,
...
class Timer(QWidget):
    ...
    def record_timer_start(self):
        cursor.execute('''INSERT INTO timers (type, time_elapsed) VALUES (?, ?)''', ('timer', self.time_left))
        conn.commit()

    def record_timer_stop(self):
        cursor.execute('''UPDATE timers SET time_elapsed = ? WHERE type = ?''', (0, 'timer'))
        conn.commit()
...
class Alarm(QWidget):
    ...
    def record_alarm_set(self):
        cursor.execute('''INSERT INTO alarms (alarm_time, status) VALUES (?, ?)''', (self.alarm_time.toString(), 'set'))
        conn.commit()

    def record_alarm_triggered(self):
        cursor.execute('''UPDATE alarms SET status = ? WHERE alarm_time = ?''', ('triggered', self.alarm_time.toString()))
        conn.commit()
...
class Pomodoro(QWidget):
    ...
    def record_pomodoro_start(self):
        cursor.execute('''INSERT INTO pomodoros (type, time_elapsed, is_working) VALUES (?, ?, ?)''', ('pomodoro', self.time_left, 1 if self.is_working else 0))
        conn.commit()

    def record_pomodoro_stop(self):
        cursor.execute('''UPDATE pomodoros SET time_elapsed = ?, is_working = ? WHERE type = ?''', (0, 0, 'pomodoro'))
        conn.commit()
